...
writeBuffer();if(pos==mask.replace("?","").length){input.caret(0,pos)}else{setTimeout(function(){input.caret(pos)},50)}},10)}).on("blur.mask",blurEvent).on("keydown.mask",keydownEvent).on("keypress.mask",keypressEvent).on("input.mask paste.mask",function(){if(input.prop("readonly")){return}
if(input.data('tilda-mask').indexOf('+7')===0){var value=input.val().replace(/\D/g,'');if(value.indexOf('78')===0){input.val(value.substring(2))}
if(value.indexOf('8')===0||value.indexOf('7')===0){input.val(value.substring(1))}}
...
break;case 'deliveryreq':error.type.push('deliveryreq');break;default:break}
...
for(j=0;j<arErrors[i].type.length;j++){error=arErrors[i].type[j];sError='';if(isShowCommonErrors==1){$errItem=$jform.find('.js-rule-error-'+error);if($errItem.length>0){if($errItem.text()==''&&t_form_dict(error)>''){if(sCommonError.indexOf(t_form_dict(error))==-1){sCommonError=sCommonError+'<p class="t-form__errorbox-item">'+t_form_dict(error)+'</p>'}}else{sError=$errItem.eq(0).text();if(sCommonError.indexOf(t_form_dict(error))==-1){sCommonError=sCommonError+'<p class="t-form__errorbox-item">'+$errItem.eq(0).text()+'</p>'}}}else{if(t_form_dict(error)>''){if(sCommonError.indexOf(t_form_dict(error))==-1){sCommonError=sCommonError+'<p class="t-form__errorbox-item">'+t_form_dict(error)+'</p>'}}}}
if(isFieldErrorBoxExist){if(sError==''){if(t_form_dict(error+'field')>''){sError=t_form_dict(error+'field')}else{if(t_form_dict(error)>''){sError=t_form_dict(error)}}}
...
var clsInputBoxSelector=$jform.data('inputbox');if(!clsInputBoxSelector){clsInputBoxSelector='.blockinput'}
var $fieldgroup,isFieldErrorBoxExist,isShowCommonErrors,$errItem,sError='';for(var i=0;i<arErrors.length;i++){if(!arErrors[i]||!arErrors[i].obj){continue}
if(i==0&&arErrors[i].obj=='none'){$errItem=$jform.find('.js-rule-error-all');$errItem.html(t_form_dict('emptyfill'));$errItem.css('display','block').show();break}
$fieldgroup=arErrors[i].obj.closest(clsInputBoxSelector).addClass('js-error-control-box')
isFieldErrorBoxExist=0;isShowCommonErrors=1;if($fieldgroup.find('.t-input-error').length>0){isFieldErrorBoxExist=1}
for(j=0;j<arErrors[i].type.length;j++){error=arErrors[i].type[j];sError='';if(isShowCommonErrors==1){$errItem=$jform.find('.js-rule-error-'+error);if(($errItem.attr('data-rule-filled'))){$errItem.css('display','block').show()}else if($errItem.length>0){if($errItem.text()==''&&t_form_dict(error)>''){$errItem.html(t_form_dict(error))}else{sError=$errItem.eq(0).text()}
$errItem.css('display','block').show()}else{if(t_form_dict(error)>''){$errItem=$jform.find('.js-rule-error-all');if($errItem&&$errItem.length>0){$errItem.html(t_form_dict(error));$errItem.css('display','block').show()}}}}
if(isFieldErrorBoxExist){if(sError==''){if(t_form_dict(error+'field')>''){sError=t_form_dict(error+'field')}else{if(t_form_dict(error)>''){sError=t_form_dict(error)}}}
...
var obj=[];$.each(window.tcart.products,function(index,product){if(!$.isEmptyObject(product)&&product.deleted!=='yes'){obj.push(product)}});window.tcart.products=obj;var dNow=new Date();var offsetFrom_UTC_to_Local=dNow.getTimezoneOffset();window.tildaForm.tildapayment.timezoneoffset=offsetFrom_UTC_to_Local;var arProduct,optionlabel,iProductsCount=0;if(window.tcart.products&&window.tcart.products.length>0){iProductsCount=window.tcart.products.length}
...
$jform.append(html)}
window.tildaForm.clearTCart=function($jform){if($jform.data('formcart')=='y'){window.clearTCart=!0;window.tcart={amount:0,currency:'',system:'',products:[]};window.tcart.system='none';if(typeof localStorage==='object'){try{localStorage.removeItem("tcart")}catch(e){console.log('Your web browser does not support localStorage.')}}
...
if(t_form_dict('successorder')){$jform.find('.js-successbox').html(t_form_dict('successorder'))}
...
html+='</form>';$('body').append(html);tildaForm.clearTCart($jform);if($('#js-tilda-payment-formid').attr('action')>''){$('#js-tilda-payment-formid').trigger('submit')}else{setTimeout(function(){$('#js-tilda-payment-formid').trigger('submit')},200)}}else{if(arNext.type=='function'){var arArgs=arNext.value.args;if(arNext.value.functioncode){tildaForm.paysystemRun(arNext.value.script,arNext.value.sysname,$jform,arNext.value.functioncode,arArgs)}else{eval(arNext.value.name+'($jform, arArgs);')}
...
if(arArgs.successmessage&&arArgs.successmessage>''){$jform.find('.js-successbox').html(arArgs.successmessage)}else{if($jform.data('successmessage')>''){$jform.find('.js-successbox').html($jform.data('successmessage'))}else{if(t_form_dict('success')){$jform.find('.js-successbox').html(t_form_dict('success'))}else{$jform.find('.js-successbox').html('')}}}
...
var successcallback=$jform.data('success-callback');if(successcallback&&successcallback.length>0&&successcallback==='t396_onSuccess'&&typeof window.t396_onSuccess==="function"){eval(successcallback+'($jform)')}else if(successcallback&&successcallback.length>0&&successcallback!=='t396_onSuccess'){eval(successcallback+'($jform)')}
$jform.find('.t-upwidget-container__data_table_actions_remove svg').click();$jform.find('input[type=text]:visible').val('');$jform.find('textarea:visible').html('');$jform.find('textarea:visible').val('');$jform.data('tildaformresult',{tranid:"0",orderid:"0"});$jform.trigger('tildaform:aftersuccess')};window.tildaForm.banktransferPay=function($jform,arArgs){if(arArgs&&arArgs.condition=='fast'){window.tildaForm.sendStatAndShowMessage($jform,arArgs,!0)}else{if(arArgs&&arArgs.html>''){$('#allrecords').append(arArgs.html);$('.t-banktransfer .t-popup__close').off('click');$('.t-banktransfer .t-popup__close').click(function(){$('body').removeClass('t-body_popupshowed');$('.t-banktransfer').remove();try{if(typeof tcart__closeCart=='function'){tcart__closeCart()}}catch(e){}
...
$jform.addClass('js-send-form-success');if(successcallback&&successcallback.length>0&&successcallback==='t396_onSuccess'&&typeof window.t396_onSuccess==="function"){eval(successcallback+'($jform)')}else if(successcallback&&successcallback.length>0&&successcallback!=='t396_onSuccess'){eval(successcallback+'($jform)')}else{if(successurl&&successurl.length>0){setTimeout(function(){window.location.href=successurl},500)}}
tildaForm.clearTCart($jform);$jform.find('.t-upwidget-container__data_table_actions_remove svg').click();$jform.find('input[type=text]:visible').val('');$jform.find('input[type=tel]:visible').val('');$jform.find('textarea:visible').html('');$jform.find('textarea:visible').val('');$jform.data('tildaformresult',{tranid:"0",orderid:"0"})};window.tildaForm.send=function($jform,btnformsubmit,formtype,formskey){window.tildaForm.tildapayment=!1;if($jform.data('formcart')=='y'||$jform.closest('.t706__orderform').length>0){window.tildaForm.addPaymentInfoToForm($jform)}
...
if(window.dataLayer){window.dataLayer.push({'event':'submit_'+$jform.attr('id')})}}else{try{if(typeof ga!='undefined'&&ga&&window.mainTracker!='tilda'){ga('send',{'hitType':'pageview','page':virtPage,'title':virtTitle})}
if(window.mainMetrika>''&&window[window.mainMetrika]){window[window.mainMetrika].hit(virtPage,{title:virtTitle,referer:window.location.href})}}catch(e){console.log(e)}
if(window.dataLayer){window.dataLayer.push({'event':'submit_'+$jform.attr('id')})}}
try{$jform.trigger('tildaform:aftersuccess');if(formsendedcallback&&formsendedcallback.length>0){eval(formsendedcallback+'($jform);')}}catch(e){console.log(e)}
...
if(!error||(error.status==0&&ts_delta<100)){$allError.html('Request error (opening block content panel). Please check internet connection...')}else{if(error&&(error.status>=500||error.status==408||error.status==410||error.status==429||error.statusText=='timeout')&&window.tildaForm.endpoint.indexOf('forms.tilda')!==-1){window.tildaForm.endpoint='forms2.tildacdn.com';window.tildaForm.send($jform,btnformsubmit,formtype,formskey);return!1}else{if(error&&error.responseText>''){$allError.html('['+error.status+'] '+error.responseText+'. Please, try again later.')}else{if(error&&error.statusText){$allError.html('Error ['+error.status+', '+error.statusText+']. Please, try again later.')}else{$allError.html('['+error.status+'] '+'Unknown error. Please, try again later.')}}}}
